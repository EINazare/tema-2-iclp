defmodule NecromancerStrategy do
  def start_link(dragon_pid) do
    Task.start_link(fn -> send_attack(dragon_pid) end)
  end
      ...
      life > 0 ->
        receive do
          {:Dragon_pid, dragon_pid} ->
            {:ok, send_attack_pid} = NecromancerStrategy.start_link(dragon_pid)
...
defmodule DragonStrategy do
  def start_link(necromancer_pid) do
    Task.start_link(fn -> send_attack(necromancer_pid) end)
  end
      ...
      life > 0 ->
        receive do
          {:Necromancer_pid, necromancer_pid} ->
            {:ok, send_attack_pid} = DragonStrategy.start_link(necromancer_pid)
